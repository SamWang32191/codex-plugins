...
sdkman_switch_jdk_target_state() {
  ...
  printf 'link-hex:'
  printf '%s' "$target" | LC_ALL=C od -An -v -tx1 | \
    LC_ALL=C tr -d '[:space:]' || return 1
  printf '\n'
}
...
sdkman_switch_jdk_reap_stale_lock() {
  ...
  rmdir "$reaper_dir" || cleanup_status=1
  rmdir "$sdkman_switch_jdk_lock_dir" || cleanup_status=1
...
sdkman_switch_jdk_create_lock() {
  ...
  umask 077
  nonce="$(
    LC_ALL=C od -An -N 16 -tx1 /dev/urandom | \
      LC_ALL=C tr -d '[:space:]'
  )" || nonce=''
  if [[ ! "$nonce" =~ ^[0-9a-f]{32}$ ]]; then
    umask "$old_umask"
    rmdir "$sdkman_switch_jdk_lock_dir" 2>/dev/null || true
  ...
  if ! printf '%s\n' "$$" > "$sdkman_switch_jdk_lock_dir/pid" || \
    ...
    umask "$old_umask"
    unlink "$sdkman_switch_jdk_lock_dir/pid" 2>/dev/null || true
    unlink "$sdkman_switch_jdk_lock_dir/euid" 2>/dev/null || true
    unlink "$sdkman_switch_jdk_lock_dir/token" 2>/dev/null || true
    unlink "$sdkman_switch_jdk_lock_dir/label" 2>/dev/null || true
    rmdir "$sdkman_switch_jdk_lock_dir" 2>/dev/null || true
...
sdkman_switch_jdk_acquire_lock() {
  ...
  while (( attempt < 2 )); do
    old_umask="$(umask)"
    umask 077
    if mkdir "$sdkman_switch_jdk_lock_dir" 2>/dev/null; then
      umask "$old_umask"
      sdkman_switch_jdk_create_lock
      return $?
    fi
...
sdkman_switch_jdk_cleanup_restore_temp() {
  ...
  if [[ -n "$sdkman_switch_jdk_restore_temp_link" && \
    ...
    unlink "$sdkman_switch_jdk_restore_temp_link" || cleanup_status=1
  fi
  if [[ -n "$sdkman_switch_jdk_restore_temp_dir" && \
        -d "$sdkman_switch_jdk_restore_temp_dir" ]]; then
    rmdir "$sdkman_switch_jdk_restore_temp_dir" || cleanup_status=1
  fi
  sdkman_switch_jdk_restore_temp_link=''
  sdkman_switch_jdk_restore_temp_dir=''
  return "$cleanup_status"
...
sdkman_switch_jdk_restore_default() {
  ...
  if [[ "$previous_state" == link-hex:* ]] && \
    ...
    sdkman_switch_jdk_restore_temp_dir="$(mktemp -d "$current_dir/.sdkman-switch-jdk-restore.XXXXXX")" || return 1
    sdkman_switch_jdk_restore_temp_link="$sdkman_switch_jdk_restore_temp_dir/current"
    if ! ln -s -- "$sdkman_switch_jdk_decoded_target" "$sdkman_switch_jdk_restore_temp_link" || \
    ...
    if mv -fh "$sdkman_switch_jdk_restore_temp_link" "$current" 2>/dev/null; then
      move_status=0
    else
      move_status=$?
    fi
    if (( move_status != 0 )) && [[ -L "$sdkman_switch_jdk_restore_temp_link" ]]; then
      ...
      if mv -Tf "$sdkman_switch_jdk_restore_temp_link" "$current" 2>/dev/null; then
        move_status=0
      else
        move_status=$?
      fi
    ...
    unlink "$current" || restore_status=1
  else
    restore_status=1
  fi
